...
const DataManager = () => {
  ...
  // Save a version snapshot before cleaning
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      {/* Preview Dialog with column operations */}
      <Dialog open={!!previewDataset} onOpenChange={() => setPreviewDataset(null)}>
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-auto">
          <DialogHeader><DialogTitle>Preview: {previewDataset?.name}</DialogTitle></DialogHeader>
          {previewDataset?.preview_rows && (
            <div className="overflow-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                    {((previewDataset.columns as string[]) || []).map((c: string) => (
                      <TableHead key={c} className="text-xs font-bold whitespace-nowrap">
                        <div className="flex items-center gap-1">
                          <span>{c}</span>
                          <Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => setRenameCol({ dsId: previewDataset.id, oldName: c, newName: c })} title="Rename">
                            <Edit3 className="w-3 h-3" />
                          </Button>
                          <Button variant="ghost" size="icon" className="h-5 w-5 text-destructive" onClick={() => deleteColumnMutation.mutate({ dsId: previewDataset.id, colName: c })} title="Delete column">
                ...
                <TableBody>
                  {((previewDataset.preview_rows as any[]) || []).slice(0, 50).map((row: any, i: number) => (
                    <TableRow key={i}>{((previewDataset.columns as string[]) || []).map((c: string) => <TableCell key={c} className="text-xs whitespace-nowrap">{String(row[c] ?? "")}</TableCell>)}</TableRow>
                  ))}
                </TableBody>
              </Table>
            </div>
